...
int main(int argc, char** argv) {
    ...
    for (const auto& entry : fs::directory_iterator(relativePath)) {
        ...
        if (entry.is_directory()) {
            ...
            vector<cv::Mat> recognizedFoodSegmented; // vector of the recognized food segmented
            ...
            for (auto& image : images) {
                ...
                // create a Mat where we'll store the final segmented image
                Mat segmented_image = Mat::zeros(image.size(), CV_8UC1);

                //START SEGMENTATION

                std::cout << "Start of peer group filtering" << std::endl;

                cv::Mat weighted_image = image.clone();

                std::vector<double> peerVec;
                cv::Mat imga = weighted_image.clone();

                peerVec = peer_group_filter(imga, weighted_image, 3);

                cv::imshow("Filtered Image", weighted_image);
                cv::waitKey(0);

                std::cout << "End of peer group filtering" << std::endl;
                cv::destroyAllWindows();

                std::cout << "Start of color quantization" << std::endl;
                cv::Mat clusterAssignments(weighted_image.rows, weighted_image.cols, CV_8UC1, cv::Scalar(0));

                int numClasses = 0;
                numClasses = quantization(weighted_image, peerVec, clusterAssignments);

                std::cout << "End of color quantization" << std::endl;
                cv::destroyAllWindows();

                std::cout << "Start of J images calculation" << std::endl;
                cv::Mat classMap(weighted_image.rows, weighted_image.cols, CV_8UC1);

                //create the class_map
                for (int i = 0; i < classMap.rows; i++) {
                    for (int j = 0; j < classMap.cols; j++) {
                        classMap.at<uchar>(i, j) = clusterAssignments.at<uchar>(i, j);
                    }
                }

                std::vector<int> windows;
                //windows.push_back(65);
                windows.push_back(33);
                windows.push_back(17);
                windows.push_back(9);

                std::vector<cv::Mat> J_imgs;
                std::vector<cv::Point> seeds;

                J_imgs = J_images(classMap, windows, numClasses);

                std::cout << "End of J images calculation" << std::endl;

                std::cout << "Start of region growing" << std::endl;

                cv::Mat regions;
                int numRegions = 0;
                regions = region_growing(J_imgs, windows, numRegions);

                std::cout << "End of region growing" << std::endl;

                //mergeRegions(regions, classMap, numRegions, 0.4);
                ...
                //HoughCircles(grayscale, circles, HOUGH_GRADIENT, 1, grayscale.rows / 2.5, 140, 55, 185, 450);
                ...
                for (size_t i = 0; i < circles.size(); i++) {
                    ...
                    Mat segPlate = regions(rec);
                    Mat segClassMap = classMap(rec);

                    mergeRegions(segPlate, segClassMap, numRegions, 0.4);

                    segmentedPlates.push_back(segPlate);
                }
                    ...
                    for (size_t i = 0; i < plates.size(); i++) {
                        ...
                        if (plates.size() > 2 && (cvRound(circles[i][2]) == smallestCircle)) {
                            ...
                            if (salad_rect != Rect(0, 0, 0, 0)) {
                                ...
                                recognizedFoodSegmented.push_back(segmentedPlates[i]);
                ...
                //Subsequently we just match what we found with the new ones
                    ...
                    for (size_t i = 0; i < plates.size(); i++) {
                        ...
                        recognizedFoodSegmented[curr_index] = segmentedPlates[i];
                    }
                }

                //Reassign the labels in the segmented image
                reassignCorrectLabels(recognizedFoodSegmented, recognizedFoodID);

                for (size_t f = 0; f < plates_box.size(); f++) {
                    if (PORK <= recognizedFoodID[f] && recognizedFoodID[f] <= SEAFOOD) {

                        segmented_image(plates_box[f]) = recognizedFoodSegmented[f];
                        f++;
                        continue;
                    }
                    segmented_image(plates_box[f]) = recognizedFoodSegmented[f];
                ...
                // write the segmentation mask into a png file
                imwrite(entry.path().string() + PREDS_SS + name + ".png", segmented_image);
